...
export function fillDb() {
  ...
  if (!db.prepare(`SELECT 1 FROM site WHERE id=1`).get()) {
    db.prepare(`
      ...
      VALUES (1, ' مبل فرحزاد فعالیت خود را با هدف جلب رضایت مشتریان گرامی خود آغاز کرده و توانسته تا با کنار هم قرار دادن کیفیت، تنوع بالا و طراحی به روز پاسخگوی سلیقه های متفاوت باشد، همچنین در صورت همراهی با بخش مقالات ما از آخرین ترندها و نحوه تلفیق المان ها در دکوراسیون داخلی آگاه خواهید شد.', 'تهران', 'info@test.com', '09120000000')
    `).run();
  }
